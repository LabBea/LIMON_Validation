---
title: "SE & LIONNESS - Dataset 2g 100/100"
output: html_notebook
---


In this script, we will be testing how well SE does at recovering individual networks with LIONESS using Dataset 2g - 100 subjects, 100 taxa. 

Note: Because R kept crashing (i think too many iterations of spieceasi?), I split it up into two parts. First find the base individualized networks and which edge to change. Save it with RDS, then close out of R. re start and load in the old object and now run the new
...
## 3.4 - Change Values
Based on these, set species 98 to 0 in subject one
```{r}
counts_altered <- counts_data
counts_altered["Sbj1_Time2", 98] <- 0

```


```{r}
#saveRDS(Dataset2g_Res, here("100sp_100taxa_half.rds"))
Dataset2g_Res <- readRDS(here("100sp_100taxa_half.rds"))
```
...
saveRDS(Dataset2g_Res, here("100sp_100taxa_full.rds"))
```
...
Save the data
```{r}
saveRDS(Dataset2g_Res, here("Output", "SE_Lionness", "Dataset2g_Res_100sbj100sp.rds"))

write.csv(Combined_data_sortededge, here("Output", "SE_Lionness", "Edges_100sbj100sp.csv"))
```
